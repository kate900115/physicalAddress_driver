		...
		for(unsigned i=0; i<state->page_count; i++) {
			...
			if (va == MAP_FAILED ) {
				...
				*writeEN = 0;
				...
				// set counter to be 0
				volatile uint64_t* reset = (uint64_t*)getAddrWithOffset(p, 0x0);
				*reset = 0;

				volatile uint64_t* counts  = (uint64_t*)getAddrWithOffset(p, 0xC);
				...
				cuCtxSynchronize();
				// this functionality should be implement on FPGA
				// e.g. when FPGA gets 100000 times write
				// it writes to a CPU memory location to notify CPU that it can end its timer.
