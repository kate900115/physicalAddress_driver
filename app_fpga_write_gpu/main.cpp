		...
		for(unsigned i=0; i<state->page_count; i++) {
			...
			if (va == MAP_FAILED ) {
				...
				*writeEN = 0;
				...
				// this part is using FPGA to do something
				// I just use CPU to control FPGA
				// You can modified FPGA code to implement such behavior
				while(countNum<10000){
					// each while iteration CPU requests FPGA to write a number into GPU
					*value = countNum;
					*writeEN = 1;
					countNum++;
				}
