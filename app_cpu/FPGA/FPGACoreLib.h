...
//    - PCIe Gen2x8 Slot Interface Architectural Specifications
//    - Pikes Peak Shell Architectural Specifications
//
// Please visit http://aka.ms/catapult for more information.

#ifndef __FPGACoreLib__
#define __FPGACoreLib__
...
#define CATAPULT_FPGA_BAR_NUM (4)

#define CATAPULT_FPGA_HOST_BUFFER_NUM (4)
#define CATAPULT_FPGA_SLOT_NUM (64)
#define CATAPULT_FPGA_DATA_BUF_SIZE_PER_SLOT (65536)
#define CATAPULT_FPGA_META_BUF_SIZE_PER_SLOT (128)
#define CATAPULT_FPGA_DATA_BUF_SIZE (CATAPULT_FPGA_SLOT_NUM * CATAPULT_FPGA_DATA_BUF_SIZE_PER_SLOT)
#define CATAPULT_FPGA_META_BUF_SIZE (CATAPULT_FPGA_SLOT_NUM * CATAPULT_FPGA_META_BUF_SIZE_PER_SLOT)
#define CATAPULT_FPGA_BAR0_SIZE (1 << 26)
#define CATAPULT_FPGA_NUM_GENERAL_PURPOSE_REGISTERS (128)

#define CATAPULT_FPGA_DEVICE_NAME_PREFIX "catapult_fpga_hip"

#define CATAPULT_FPGA_MMAP_SHIFT_BITS 24
...
#define FPGA_STATUS_UNKNOWN_FLASH_SLOT              82
#define FPGA_STATUS_EMPTY_FLASH_SLOT                83
#define FPGA_STATUS_ILLEGAL_BUFFER_RANGE            84
#define FPGA_STATUS_EXCEEDED_MAXIMUM_BUFFER_RANGE   85
...
//
// This function blocks the calling thread until a message is received from the FPGA on one of a given range of output buffers.
// The user must provide storage for the FPGA-generated PCIe header, and must also specify whether an interrupt is
// expected or polling should be used.  
// The timeout parameter, when overriden from the default, will cause the calling thread to unblock
// after a specified duration (in seconds). Setting the timeout to zero will result in an INFINITE wait.
//  
// Slots are checked in sequence order between startBuffer and endBuffer, starting from first.  Generally the user 
// will want to set 'first' equal to (the last slot that received a message + 1) on successive calls to this 
// function to avoid starvation of higher numbered slots.
//
// On a timeout, a FPGA_STATUS_WAIT_TIMEOUT is returned. Otherwise,
// the function returns FPGA_STATUS_SUCCESS. 
// 
// If no timeout or error occurs, whichBuffer will be set with the first buffer that receives a message.
//
// pBytesReceived can receive a NULL argument, if the user wishes to discard the total number of bytes received.
// On a successful return, the function returns the total number of bytes received from the FPGA. 
//
_CDLLEXPORT_ FPGA_STATUS FPGA_WaitAnyOutputBuffer(_In_ FPGA_HANDLE fpgaHandle, _In_ DWORD startBuffer, _In_ DWORD endBuffer, _In_ DWORD first, _Out_ DWORD *whichBuffer, _Out_ DWORD *pBytesReceived, _In_ BOOL useInterrupt = TRUE, _In_ double timeoutInSeconds = FPGA_WAIT_OUTPUT_BUFFER_TIMEOUT_DEFAULT_SECONDS);
...
//These functions are exposed only for use by factory test code, the set of registers that can be accessed is internally restricted and applications must not take a dependence on them
